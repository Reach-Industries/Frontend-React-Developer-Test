...
- Annotation data in JSON format describing the position and dimensions of a rect, frame-by-frame, formatted as `[x, y, width, height]` following the hand movements [here](https://reach-industries-candidate-tests.s3.eu-west-2.amazonaws.com/FrontendCandidateTest-FINAL.json). Note: `x` and `y` describe the top left point of the rect and the data is based on the video being shown in its true size of 1280x720
